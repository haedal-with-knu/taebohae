...
### 모듈을 설치하는 방법
  ...
  2. 명령어로 설치하는 방법
    1) 설치하려는 모듈을 구글링합니다.
    2) 모듈을 설치하는 명령어를 찾아서 입력합니다(현재경로와 무관하게 치면 됩니다.)  
    (대부분 pip install (모듈 이름) 형식으로 설치가 가능하지만 가끔씩 형식이 다른 경우도 있기때문에 구글링하는 것을 추천드려요)    
    (Opencv의 경우 : pip install Opencv-python)    
...
#### 사용 예시  
    ...
    <cmd창> python d:\argvTest.py arg1 arg2 를 입력  
